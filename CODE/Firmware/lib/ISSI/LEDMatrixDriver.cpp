...
LEDMatrixDriver::LEDMatrixDriver(uint8_t sdaPin, uint8_t sdaPin, uint8_t addr, uint8_t enablePin, uint8_t* fb):
...
void LEDMatrixDriver::setPixel(int16_t x, int16_t y, uint8_t pwm)
{
...
uint8_t LEDMatrixDriver::getPixel(int16_t x, int16_t y) const
{
	return frameBuffer[x][y];
}

void LEDMatrixDriver::setColumn(int16_t x, uint8_t value) //not needed?
{
...
void LEDMatrixDriver::_sendCommand(uint8_t Reg_Add, uint8_t Reg_Add)
{
...
void LEDMatrixDriver::_drawPixel(uint8_t row, uint8_t collumn){
...
void LEDMatrixDriver::scroll(scrollDirection direction, bool wrap)
{
	switch( direction )
	{
		case scrollDirection::scrollUp:
		{
			...
			memmove(frameBuffer, frameBuffer + 39, 8*N);	//shift 7 rows
